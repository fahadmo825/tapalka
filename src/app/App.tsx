...
function App() {
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'friends') return <section className={styles.page}><p className={styles.eyebrow}>SHARE THE ENERGY</p><h1>Invite friends</h1><p className={styles.muted}>Your Telegram ID: <strong>{userId}</strong></p><div className={styles.referralCard}><span className={styles.referralIcon}>↗</span><strong>Your invite link</strong><div className={styles.linkBox}>{referralLink}</div><button className={styles.goldButton} onClick={() => navigator.clipboard?.writeText(referralLink)}>Copy invite link</button></div><div className={styles.statsRow}><div><strong>{referralCount}</strong><small>Total Referrals</small></div><div><strong>{referralEarned.toFixed(2)}</strong><small>Total AGEN earned</small></div></div><div className={styles.referralList}>{referrals.length ? referrals.map((referral) => <div key={referral.telegram_id}><span>Telegram user</span><strong>{referral.telegram_id}</strong></div>) : <p className={styles.muted}>No invitees yet.</p>}</div></section>;
